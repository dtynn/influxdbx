		...
		case <-ticker.C:
			...
			if err != nil {
				if !raft.IsErrNotLeader(err) {
					s.logger.Info(fmt.Sprintf("Failed to get databases from MetaClient: %s", err))
				}
				continue
			}
			...
			shardIDs, err := s.TSDBStore.ShardIDs()
			if err != nil {
				if !raft.IsErrNotLeader(err) {
					s.logger.Info(fmt.Sprintf("Failed to get shard ids from TSDBStore: %s", err))
				}
				continue
			}
