...
import (
	...
	metapb "github.com/dtynn/influxdbx/internal/pb"
...
func (data *Data) CreateDataNode(id uint64, address string) error {
	for _, mn := range data.DataNodes {
		if mn.ID == id {
			// conflict meta node register
			if mn.Address != address {
				return ErrMetaNodeExists
...
// RemoveDataNode removes a node from the Meta store.
//
// If necessary, RemoveDataNode reassigns ownership of any shards that
// would otherwise become orphaned by the removal of the node from the
// cluster.
func (data *Data) RemoveDataNode(id uint64) error {
	if id == 0 {
		return ErrNodeIDRequired
	}
	...
	for di, d := range data.Databases {
		for rpi, rp := range d.RetentionPolicies {
			for sgi, sg := range rp.ShardGroups {
				...
				for si, s := range sg.Shards {
					...
					if nodeIdx > -1 {
						...
						data.Databases[di].RetentionPolicies[rpi].ShardGroups[sgi].Shards[si].Owners = s.Owners
					}
				...
				if len(sg.Shards) == 0 || len(orphanedShards) == len(sg.Shards) {
					data.Databases[di].RetentionPolicies[rpi].ShardGroups[sgi].DeletedAt = time.Now().UTC()
					continue
				}
				...
				for _, orphan := range orphanedShards {
					...
					for si, s := range sg.Shards {
						if s.ID == orphan.ID {
							...
							data.Databases[di].RetentionPolicies[rpi].ShardGroups[sgi].Shards = sg.Shards
							break
						}
					}
...
// CloneDataNodes returns a copy of the data ndoes
func (data *Data) CloneDataNodes() []NodeInfo {
	if data.DataNodes == nil {
		return nil
	}

	nodes := make([]NodeInfo, len(data.DataNodes))
	for i := range data.DataNodes {
		nodes[i] = data.DataNodes[i].clone()
	}
...
func (data *Data) marshal() *metapb.MetaData {
	pb := &metapb.MetaData{
		Term:  data.Term,
		Index: data.Index,
	...
	pb.Databases = make([]*metapb.MetaDatabaseInfo, len(data.Databases))
	for i := range data.Databases {
		pb.Databases[i] = data.Databases[i].marshal()
	}

	pb.Users = make([]*metapb.MetaUserInfo, len(data.Users))
	for i := range data.Users {
...
func (data *Data) unmarshal(pb *metapb.MetaData) {
...
// MarshalBinary encodes node info to a binary format
func (n *NodeInfo) MarshalBinary() ([]byte, error) {
	return proto.Marshal(n.marshal())
}

// UnmarshalBinary decodes node info from a binary format.
func (n *NodeInfo) UnmarshalBinary(buf []byte) error {
	var pb metapb.MetaNodeInfo
	if err := proto.Unmarshal(buf, &pb); err != nil {
		return err
	}
...
func (n NodeInfo) marshal() *metapb.MetaNodeInfo {
	return &metapb.MetaNodeInfo{
		ID:      n.ID,
		Address: n.Address,
	}
}

func (n *NodeInfo) unmarshal(pb *metapb.MetaNodeInfo) {
	n.ID = pb.GetID()
	n.Address = pb.GetAddress()
}
...
func (di DatabaseInfo) marshal() *metapb.MetaDatabaseInfo {
	pb := &metapb.MetaDatabaseInfo{
		Name: di.Name,
		DefaultRetentionPolicy: di.DefaultRetentionPolicy,
	}

	pb.RetentionPolicies = make([]*metapb.MetaRetentionPolicyInfo, len(di.RetentionPolicies))
	for i := range di.RetentionPolicies {
		pb.RetentionPolicies[i] = di.RetentionPolicies[i].marshal()
	}

	pb.ContinuousQueries = make([]*metapb.MetaContinuousQueryInfo, len(di.ContinuousQueries))
	for i := range di.ContinuousQueries {
...
// unmarshal deserializes from a protobuf representation.
func (di *DatabaseInfo) unmarshal(pb *metapb.MetaDatabaseInfo) {
	di.Name = pb.GetName()
...
// marshal serializes to a protobuf representation.
func (s *RetentionPolicySpec) marshal() *metapb.MetaRetentionPolicySpec {
	pb := &metapb.MetaRetentionPolicySpec{}
	if s.Name != "" {
		pb.Name = &metapb.Str{
			Val: s.Name,
		}
	...
	if s.Duration != nil {
		pb.Duration = &metapb.I64{
			Val: int64(*s.Duration),
		}
	}

	if s.ShardGroupDuration > 0 {
		pb.ShardGroupDuration = &metapb.I64{
			Val: int64(s.ShardGroupDuration),
		}
	}

	if s.ReplicaN != nil {
		pb.ReplicaN = &metapb.I64{
			Val: int64(*s.ReplicaN),
		}
	}
...
// unmarshal deserializes from a protobuf representation.
func (s *RetentionPolicySpec) unmarshal(pb *metapb.MetaRetentionPolicySpec) {
	if pb.GetName() != nil {
		s.Name = pb.GetName().GetVal()
	}

	if pb.GetDuration() != nil {
		duration := time.Duration(pb.GetDuration().GetVal())
		s.Duration = &duration
	}

	if pb.GetShardGroupDuration() != nil {
		s.ShardGroupDuration = time.Duration(pb.GetShardGroupDuration().GetVal())
	}

	if pb.GetReplicaN() != nil {
		replicaN := int(pb.GetReplicaN().GetVal())
		s.ReplicaN = &replicaN
	}
}
...
func (s *RetentionPolicySpec) UnmarshalBinary(data []byte) error {
	var pb metapb.MetaRetentionPolicySpec
...
// marshal serializes to a protobuf representation.
func (rpi *RetentionPolicyInfo) marshal() *metapb.MetaRetentionPolicyInfo {
	pb := &metapb.MetaRetentionPolicyInfo{
		Name:               rpi.Name,
		ReplicaN:           int64(rpi.ReplicaN),
		Duration:           int64(rpi.Duration),
		ShardGroupDuration: int64(rpi.ShardGroupDuration),
	}

	pb.ShardGroups = make([]*metapb.MetaShardGroupInfo, len(rpi.ShardGroups))
	for i, sgi := range rpi.ShardGroups {
		pb.ShardGroups[i] = sgi.marshal()
	}

	pb.Subscriptions = make([]*metapb.MetaSubscriptionInfo, len(rpi.Subscriptions))
...
// unmarshal deserializes from a protobuf representation.
func (rpi *RetentionPolicyInfo) unmarshal(pb *metapb.MetaRetentionPolicyInfo) {
	rpi.Name = pb.GetName()
...
func (rpi *RetentionPolicyInfo) UnmarshalBinary(data []byte) error {
	var pb metapb.MetaRetentionPolicyInfo
	if err := proto.Unmarshal(data, &pb); err != nil {
		return err
	}
	rpi.unmarshal(&pb)
...
// marshal serializes to a protobuf representation.
func (sgi *ShardGroupInfo) marshal() *metapb.MetaShardGroupInfo {
	pb := &metapb.MetaShardGroupInfo{
		ID:          sgi.ID,
		StartTime:   MarshalTime(sgi.StartTime),
		EndTime:     MarshalTime(sgi.EndTime),
		DeletedAt:   MarshalTime(sgi.DeletedAt),
		TruncatedAt: MarshalTime(sgi.TruncatedAt),
	}

	pb.Shards = make([]*metapb.MetaShardInfo, len(sgi.Shards))
	for i := range sgi.Shards {
...
// unmarshal deserializes from a protobuf representation.
func (sgi *ShardGroupInfo) unmarshal(pb *metapb.MetaShardGroupInfo) {
	sgi.ID = pb.GetID()
...
// marshal serializes to a protobuf representation.
func (si ShardInfo) marshal() *metapb.MetaShardInfo {
	pb := &metapb.MetaShardInfo{
		ID: si.ID,
	}

	pb.Owners = make([]*metapb.MetaShardOwner, len(si.Owners))
	for i := range si.Owners {
...
// unmarshal deserializes from a protobuf representation.
func (si *ShardInfo) unmarshal(pb *metapb.MetaShardInfo) {
	si.ID = pb.GetID()
...
// marshal serializes to a protobuf representation.
func (si SubscriptionInfo) marshal() *metapb.MetaSubscriptionInfo {
	pb := &metapb.MetaSubscriptionInfo{
		Name: si.Name,
		Mode: si.Mode,
	}
...
// unmarshal deserializes from a protobuf representation.
func (si *SubscriptionInfo) unmarshal(pb *metapb.MetaSubscriptionInfo) {
	si.Name = pb.GetName()
	si.Mode = pb.GetMode()
...
// marshal serializes to a protobuf representation.
func (so ShardOwner) marshal() *metapb.MetaShardOwner {
	return &metapb.MetaShardOwner{
		NodeID: so.NodeID,
	}
}

// unmarshal deserializes from a protobuf representation.
func (so *ShardOwner) unmarshal(pb *metapb.MetaShardOwner) {
...
// marshal serializes to a protobuf representation.
func (cqi ContinuousQueryInfo) marshal() *metapb.MetaContinuousQueryInfo {
	return &metapb.MetaContinuousQueryInfo{
		Name:  cqi.Name,
		Query: cqi.Query,
	}
}
...
// unmarshal deserializes from a protobuf representation.
func (cqi *ContinuousQueryInfo) unmarshal(pb *metapb.MetaContinuousQueryInfo) {
	cqi.Name = pb.GetName()
...
func (u *UserInfo) AuthorizeDatabase(privilege influxql.Privilege, database string) bool {
	if u.Admin || privilege == influxql.NoPrivileges {
		return true
	}
...
// AuthorizeQuery used to limit query
...
// marshal serializes to a protobuf representation.
func (u UserInfo) marshal() *metapb.MetaUserInfo {
	pb := &metapb.MetaUserInfo{
		Name:  u.Name,
		Hash:  u.Hash,
		Admin: u.Admin,
	}
	...
	for database, privilege := range u.Privileges {
		pb.Privileges = append(pb.Privileges, &metapb.MetaUserPrivilege{
			Database:  database,
...
// unmarshal deserializes from a protobuf representation.
func (u *UserInfo) unmarshal(pb *metapb.MetaUserInfo) {
	u.Name = pb.GetName()
	u.Hash = pb.GetHash()
	u.Admin = pb.GetAdmin()
	...
	for _, p := range pb.GetPrivileges() {
		u.Privileges[p.GetDatabase()] = influxql.Privilege(p.GetPrivilege())
	}
}
